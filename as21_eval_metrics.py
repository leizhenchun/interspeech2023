...
def eval_to_score_file_LA(score_file, asv_key_file, asv_scr_file, cm_key_file, cost_model, phase):
    ...
    out_data = "[phase:{} min_tDCF: {:.6f}      eer: {:.6f} %".format((phase + ']').ljust(15), min_tDCF, 100 * eer_cm)
...
def eval_to_score_file_DF(score_file, cm_key_file, phase):
    ...
    spoof_cm = cm_scores[cm_scores[5] == 'spoof']['1_x'].values
    eer_cm = compute_eer(bona_cm, spoof_cm)[0]
    out_data = "[phase:{} eer: {:8.5f} %".format((phase + ']').ljust(15), 100 * eer_cm)
    logging.info(out_data)
    # print(out_data)
    return eer_cm
...
def eval_to_score_file_PA(score_file, asv_key_file, asv_scr_file, cm_key_file, cost_model, phase):
    ...
    out_data = "[phase:{} min_tDCF: {:8.5f}      eer: {:8.5f} %".format((phase + ']').ljust(15), min_tDCF, 100 * eer_cm)
    logging.info(out_data)
...
if __name__ == "__main__":
    ...
    compute_tDCF_EER21(
        '/home/lzc/lzc/ASVspoof2021/ASVspoof2021exp/GMM_ResNet_MPath_LFCC/20211128_075048_GMM_ResNet_MPath_LFCC_LA/AS21_GMM_ResNet_MPath_LFCC_LA_20211128_075048_score.txt',
        'LA')
