...
class ASVspoofWaveDataset(ASVSpoofDataset):
    def __init__(self, feature_file_path, feature_file_list, file_extension='.flac', label_id=None, feat_length=64600,
                 num_channels=0, transpose=False, keep_in_memory=False, sample_ratio=1.0, feat_transformer_fn=None,
                 dtype=np.float32):
        super(ASVspoofWaveDataset, self).__init__(feature_file_path, feature_file_list, file_extension=file_extension,
                                                  label_id=label_id,
                                                  feat_length=feat_length, num_channels=num_channels,
                                                  transpose=transpose, keep_in_memory=keep_in_memory,
...
class ASVspoofWaveAugmentDataset(ASVSpoofDataset):
    ...
    def __getitem__(self, idx):
        ...
        data = cut_wave(data, max_len=self.feat_length)
        data = np.expand_dims(data, 0)

        data = self.augment_audio(data)

        data = torch.FloatTensor(data.squeeze(0))
    ...
    # def segmentWAV(self, audio, max_frames, evalmode=True, num_eval=10):
    #
    #     # Maximum audio length
    #     max_audio = max_frames * 160 + 240
    #
    ...
    def augment_audio(self, audio):
        ...
        # elif augtype == 1:
        #     audio = audio.squeeze()
        #     augmented_audio = apply_rb1(audio, 16000)
        #     augmented_audio = apply_rb2(augmented_audio)
        #     augmented_audio = apply_rb3(augmented_audio, 16000)
        #     augmented_audio = augmented_audio[numpy.newaxis, :]
        #
        # return augmented_audio

        augtype = random.randint(0, 4)
        if augtype == 1:
            audio = self.augmenter.reverberate(audio)
        elif augtype == 2:
            audio = self.augmenter.additive_noise('music', audio)
        elif augtype == 3:
            audio = self.augmenter.additive_noise('speech', audio)
        elif augtype == 4:
            audio = self.augmenter.additive_noise('noise', audio)
