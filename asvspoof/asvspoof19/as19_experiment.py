...
from asvspoof.as_dataset import get_augment_dataset, ASVSpoofDataset
...
class AS19Experiment:
    def __init__(self, model_type, feature_type, access_type, parm=get_parameter()):
        ...
        self.dataset_cls = ASVSpoofDataset
        self.feat_transformer_fn = None
        self.dtype = parm['dtype']
        if self.dtype == np.float64:
            torch.set_default_dtype(torch.float64)

        self.root_path = parm['asvspoof_root_path']
        if not (self.root_path.endswith('/') or self.root_path.endswith('\\')):
    ...
    def _path_init(self):
        ...
        self.exp_root_path = os.path.join(self.root_path, 'ASVspoof2019exp')
        self.exp_path = os.path.join(self.exp_root_path, mf, self.exp_time + '_' + mfa)
    ...
    def train(self):
        ...
        train_dataset = get_augment_dataset(dataset_cls=self.dataset_cls,
    ...
    def train_model(self, model, train_loader):
        ...
        if self.validate_dev_epoch:
            ...
            dev_dataset = get_augment_dataset(dataset_cls=self.dataset_cls,
    ...
    def test(self, model, test_type):
        ...
        if 'test_data_basic' in self.parm and self.parm['test_data_basic']:
            ...
            test_dataset = self.dataset_cls(feature_file_path=test_feat_path,
    ...
    def get_scheduler(self, optimizer):
        return ReduceLROnPlateau(optimizer, patience=5, factor=0.1, min_lr=self.parm['min_lr'], verbose=True)
